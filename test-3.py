...
# 设置目标网址
url = ['https://imagingmall.com/jumpshop/EventPhoto?EventId=00019',
       'https://imagingmall.com/jumpshop/EventPhoto?EventId=00019&page=2&SearchTab=0&IsResearch=True&FaceSortFlag=False#pagetop']  # 替换成你要抓取的实际网站URL
    ...
    for i in range(len(url)):
        ...
        for img_tag in img_tags:
            ...
            if not img_url.startswith('http'):
                img_url = requests.compat.urljoin(url[i], img_url)
            
            # 下载图片并保存
            img_file = download_image(img_url, img_number)
            if img_file:
                image_data.append((img_file))
            img_number += 1
        image_data = image_data[1:-1]
        # image_data_final = [[i+1 for i in range(len(image_data_final)), image_data_final[img_file]]]
        for j in range(len(image_data)):
            image_data_final.append(image_data[j])

    
    # 创建 Word 文件
    create_word_document(image_data_final, 'chain_comic.docx')
